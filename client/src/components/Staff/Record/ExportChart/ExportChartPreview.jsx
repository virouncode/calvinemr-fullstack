...
const ExportChartPreview = ({
  ...
  const reportsFilesRef = useRef([]);
  const reportsTextRef = useRef(null);
  const prescriptionsFilesRef = useRef([]);
  const eformsFilesRef = useRef([]);
  const lettersFilesRef = useRef([]);
  ...
  const handleGeneratePDF = async () => {
    try {
      ...
      if (prescriptionsFilesRef.current.length > 0) {
        ...
        for (let file of prescriptionsFilesRef.current) {
          const url = file.url;
      ...
      if (reportsFilesRef.current.length > 0) {
        ...
        for (let file of reportsFilesRef.current) {
          const url = file.url;
          const arrayBuffer = await fetch(url).then((res) => res.arrayBuffer());
          if (file.mime.includes("png")) {
            const image = await pdfDoc.embedPng(arrayBuffer);
      ...
      if (eformsFilesRef.current.length > 0) {
        ...
        for (let file of eformsFilesRef.current) {
          const url = file.url;
          const arrayBuffer = await fetch(url).then((res) => res.arrayBuffer());
          if (file.mime.includes("png")) {
            const image = await pdfDoc.embedPng(arrayBuffer);
      ...
      if (lettersFilesRef.current.length > 0) {
        ...
        for (let file of lettersFilesRef.current) {
          const url = file.url;
          const arrayBuffer = await fetch(url).then((res) => res.arrayBuffer());
          if (file.mime.includes("png")) {
            const image = await pdfDoc.embedPng(arrayBuffer);
