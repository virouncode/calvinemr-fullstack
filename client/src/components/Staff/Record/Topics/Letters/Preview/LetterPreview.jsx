...
const LetterPreview = ({
  ...
  const [recordInfosPages, setRecordInfosPages] = useState([]);
  const [recordInfosAdditionalBodies, setRecordInfosAdditionalBodies] =
    useState([]);
  const printRecordInfosRef = useRef(null);
  const printRecordInfosAdditionalRefs = useRef([]);
  ...
  const handleSave = async (e) => {
    ...
    for (let printRecordInfosAdditionalRef of printRecordInfosAdditionalRefs.current) {
  ...
  const handleSend = async (e, type) => {
    ...
    let fileToUploadLetterForMessage = await xanoPost(
      "/upload/attachment",
      "staff",
      {
        content: pdfURI,
    ...
    if (letter) {
      ...
      let fileToUploadLetterForRecord = await xanoPost(
        "/upload/attachment",
        "staff",
        {
