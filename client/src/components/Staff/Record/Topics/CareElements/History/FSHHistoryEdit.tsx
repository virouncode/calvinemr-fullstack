...
const FSHHistoryEdit = ({
  ...
  const [formDatasFSH, setFormDatasFSH] = useState<
    {
      id: number;
      FSH: string;
      FSHUnit: "IU/L";
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch (name) {
      case "FSH":
        setFormDatasFSH(
          formDatasFSH.map((item) => {
            return item.id === id ? { ...item, FSH: value } : item;
          })
        );
        break;
      case "Date":
  ...
  const handleSubmit = async () => {
    //Validation
    if (formDatasFSH.some(({ FSH, Date }) => !FSH || !Date)) {
      setErrMsgPost("Please fill both FSH and Date fields");
      return;
    }
    if (formDatasFSH.some(({ FSH }) => !FSH.match(/^\d+(\.\d{0,2})?$/))) {
      setErrMsgPost("Please enter a valid number for FSH");
      return;
    }
        ...
        {formDatasFSH.map((item) => (
          <li className="care-elements__edit-item" key={item.id}>
            <span className="care-elements__edit-block care-elements__edit-block--double">
              <InputDate
                ...
                value={timestampToDateISOTZ(
                  formDatasFSH.find(({ id }) => id === item.id)?.Date
                )}
            ...
            <span className="care-elements__edit-block care-elements__edit-block--double">
              <Input
                label="FSH (IU/L):"
                value={formDatasFSH.find(({ id }) => id === item.id)?.FSH ?? ""}
                onChange={handleChange}
                id={item.id.toString()}
                name="FSH"
              />
