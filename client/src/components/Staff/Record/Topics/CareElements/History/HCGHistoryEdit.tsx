...
const HCGHistoryEdit = ({
  ...
  const [formDatasHCG, setFormDatasHCG] = useState<
    {
      id: number;
      HCG: string;
      HCGUnit: "IU/L";
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch (name) {
      case "HCG":
        setFormDatasHCG(
          formDatasHCG.map((item) => {
            return item.id === id ? { ...item, HCG: value } : item;
          })
        );
        break;
      case "Date":
  ...
  const handleSubmit = async () => {
    //Validation
    if (formDatasHCG.some(({ HCG, Date }) => !HCG || !Date)) {
      setErrMsgPost("Please fill both HCG and Date fields");
      return;
    }
    if (formDatasHCG.some(({ HCG }) => !HCG.match(/^\d+(\.\d{0,2})?$/))) {
      setErrMsgPost("Please enter a valid number for HCG");
      return;
    }
        ...
        {formDatasHCG.map((item) => (
          <li className="care-elements__edit-item" key={item.id}>
            <span className="care-elements__edit-block care-elements__edit-block--double">
              <InputDate
                ...
                value={timestampToDateISOTZ(
                  formDatasHCG.find(({ id }) => id === item.id)?.Date
                )}
            ...
            <span className="care-elements__edit-block care-elements__edit-block--double">
              <Input
                label="HCG (IU/L):"
                value={formDatasHCG.find(({ id }) => id === item.id)?.HCG ?? ""}
                onChange={handleChange}
                id={item.id.toString()}
                name="HCG"
              />
