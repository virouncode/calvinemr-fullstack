...
const TSHHistoryEdit = ({
  ...
  const [formDatasTSH, setFormDatasTSH] = useState<
    {
      id: number;
      TSH: string;
      TSHUnit: "uIU/mL";
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch (name) {
      case "TSH":
        setFormDatasTSH(
          formDatasTSH.map((item) => {
            return item.id === id ? { ...item, TSH: value } : item;
          })
        );
        break;
      case "Date":
  ...
  const handleSubmit = async () => {
    //Validation
    if (formDatasTSH.some(({ TSH, Date }) => !TSH || !Date)) {
      setErrMsgPost("Please fill both TSH and Date fields");
      return;
    }
    if (formDatasTSH.some(({ TSH }) => !TSH.match(/^\d+(\.\d{0,2})?$/))) {
      setErrMsgPost("Please enter a valid number for TSH");
      return;
    }
        ...
        {formDatasTSH.map((item) => (
          <li className="care-elements__edit-item" key={item.id}>
            <span className="care-elements__edit-block care-elements__edit-block--double">
              <InputDate
                ...
                value={timestampToDateISOTZ(
                  formDatasTSH.find(({ id }) => id === item.id)?.Date
                )}
            ...
            <span className="care-elements__edit-block care-elements__edit-block--double">
              <Input
                label="TSH (uIU/mL):"
                value={formDatasTSH.find(({ id }) => id === item.id)?.TSH ?? ""}
                onChange={handleChange}
                id={item.id.toString()}
                name="TSH"
              />
