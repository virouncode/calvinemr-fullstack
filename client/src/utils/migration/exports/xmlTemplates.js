...
<cdsd:NamePrefix>${escapeXml(jsObj.Names?.NamePrefix ?? "")}</cdsd:NamePrefix>
      ...
      ? jsObj.PhoneNumber.map(
        ...
        ${
          item.phoneNumber
            ...
            : `<cdsd:areaCode>${escapeXml(item.areaCode ?? "")}</cdsd:areaCode>
              <cdsd:number>${escapeXml(item.number ?? "")}</cdsd:number>
              <cdsd:extension>${escapeXml(
                item.extension ?? ""
              )}</cdsd:extension>
              <cdsd:exchange>${escapeXml(item.exchange ?? "")}</cdsd:exchange>`
        }
</PhoneNumber>`
        ).join("")
      : "";
  ...
  const xmlpreferred_pharmacy = patientInfos.preferred_pharmacy
    ? `<preferred_pharmacy>
    ...
    <FaxNumber phoneNumberType=${escapeXml(
      jsObj.preferred_pharmacy?.FaxNumber?._phoneNumberType
       ...
       ${
         jsObj.preferred_pharmacy?.FaxNumber?.phoneNumer
           ? `<cdsd:phoneNumber>${escapeXml(
               jsObj.preferred_pharmacy?.FaxNumber?.phoneNumer
             )}</cdsd:phoneNumber>
        <cdsd:extension>${escapeXml(
          jsObj.preferred_pharmacy?.FaxNumber?.extension ?? ""
           ...
           : `<cdsd:areaCode>${escapeXml(
               jsObj.preferred_pharmacy?.FaxNumber.areaCode ?? ""
             )}</cdsd:areaCode>
        <cdsd:number>${escapeXml(
          jsObj.preferred_pharmacy?.FaxNumber.number ?? ""
        )}</cdsd:number>
         <cdsd:extension>${escapeXml(
           jsObj.preferred_pharmacy?.FaxNumber.extension ?? ""
         )}</cdsd:extension>
        <cdsd:exchange>${escapeXml(
          jsObj.preferred_pharmacy?.FaxNumber.exchange ?? ""
        )}</cdsd:exchange>`
       }
    </FaxNumber>
...
export const toXmlReports = (jsObj) => {
  ...
  ${
    jsObj.SentDateTime
      ? escapeXml(timestampToDateTimeSecondsISOTZ(jsObj.SentDateTime))
      : ""
  }
  </cdsd:FullDateTime>
</SentDateTime>`;
...
<cdsd:Content>${escapeXml(jsObj.term_nbr_of_weeks ?? "")}</cdsd:Content>
</cdsd:DataElement>`
    : ""
}
${
