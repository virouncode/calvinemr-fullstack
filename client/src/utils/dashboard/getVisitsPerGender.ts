...
export const getVisitsPerGender = (
  sites: SiteType[],
  ...
  for (const site of sites) {
    ...
    for (const visit of visitsForSite) {
      ...
      ).filter(({ patient_infos }) => patient_infos?.Gender === "M").length;
      nbrOfFemale += (
        visit.patients_guests_ids as { patient_infos: DemographicsType }[]
      ).filter(({ patient_infos }) => patient_infos?.Gender === "F").length;
      nbrOfOther += (
        visit.patients_guests_ids as { patient_infos: DemographicsType }[]
      ).filter(({ patient_infos }) => patient_infos?.Gender === "O").length;
    }
    totalsBySite = [
      ...totalsBySite,
